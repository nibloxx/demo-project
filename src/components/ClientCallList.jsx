...
export function ClientCallList() {
  ...
  useEffect(() => {
    ...
    const pauseDuration = 1200; // pause for ~1.2s
    const scrollSpeed = 0.8; // smaller = smoother
  ...
  return (
    <div
      className="max-w-full mx-auto overflow-hidden -mt-36 relative font-open-runde"
    ...
    >
      <div className="space-y-8 animate-scroll">
        {groupedCalls.map(([date, calls]) => (
          <div
            key={date}
            className="space-y-4 bg-white border border-gray-100 rounded-xl shadow-sm w-80"
